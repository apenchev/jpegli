...
* Install dependencies by running ./deps.sh

* Install CMake and Emscripten SDK by following the instructions on https://github.com/apenchev/jpegli/blob/main/doc/building_wasm.md but DO NOT RUN the last step in the docs (BUILD_TARGET=wasm32 ENABLE_WASM_SIMD=1 emconfigure ./ci.sh release)

* Instead, you first need to log on as root with: sudo su

* Onced logged on as root, you need to configure the Emscripten SDK again with: source $OPT/emsdk/emsdk_env.sh

* Finally, you can run: BUILD_TARGET=wasm32 ENABLE_WASM_SIMD=1 SKIP_TEST=1 emconfigure ./ci.sh release
